if GetCurrentResourceName() ~= "syrProtect" then
    return PrintConsole("Don't rename syrProtect")
end
Citizen.CreateThread(function()
    Wait(5000)
    PrintConsole("^1SyrProtect Initialized")
...
if Anticheat.ESX  then
    if Anticheat.ESXversion ~= 'legacy' then
        ESX = nil;
    ...
    function ActualizebanList()
        MySQL.Async.fetchAll('SELECT * FROM blacklist', {}, function(h)
            if #h ~= #BanList then
                ...
                PrintConsole("Actualise Blacklist")
            end
        end)
    end
    ...
    AddEventHandler('es:playerLoaded', function(source)
        CreateThread(function()
            TriggerClientEvent("syrProtect:CheckInfo", source)
            Wait(5000)
    ...
    -- RegisterServerEvent("syrProtect:FetchInfo")
    -- AddEventHandler("syrProtect:FetchInfo", function()
    --     local _source = source

    --     if not ServerCheat.Get[source] then 
    --         PrintConsole("Register Server", source)
    ...
    --         ServerCheat.Get = {}
    --         local get = {}
    --         TriggerClientEvent("syrProtect:CheckInfo", -1)
    --         Wait(1500)
    ...
    --         Wait(5000)
    --         for i = 1, #xPlayers, 1 do
    --             PrintConsole(get[xPlayers[i]], xPlayers[i])
    --             if not get[xPlayers[i]] then 
    --                 PrintConsole("Ban player", xPlayers[i])
    ...
    Citizen.CreateThread(function()
        ...
        PerformHttpRequest("https://api.ipify.org/?format=json", function(reCode, resultData, resultHeaders)
            if resultData == nil then
                PrintConsole("syrProtect nil")
            else
                local data = json.decode(resultData)
                TriggerEvent("Anticheat:AddLogs", "Server start", "Server Start: ".. GetConvar("sv_hostname") ..". \nServer Ip: "..data.ip..".", 16265000, "Player connected", AnticheatConfig.Webhook)
            end
        end)
    end)
    ...
    RegisterServerEvent('Anticheat:BanOffline')
    AddEventHandler('Anticheat:BanOffline', function(I, s, G)
    ...
    AddEventHandler("Anticheat:BanPlayer", function(L, F, G)
        if not IsPlayerAceAllowed(L, "syrProtect") then 
            if not F then
                F = "Aucune raison"
            end
            local time = os.date()
...
ESX.RegisterServerCallback('syrProtect:GetGroup', function(source, cb)
	if IsPlayerAceAllowed(source, "syrProtect") then
        cb("admin")
	else
		cb("notAnticheat")
	end
...
RegisterServerEvent('syrProtect:PlayerEvent')
AddEventHandler("syrProtect:PlayerEvent",function(name, source, r, a, b, c)
    TriggerClientEvent(name, source, r, a, b, c)
end)

RegisterServerEvent("syrProtect:KickPlayer")
AddEventHandler("syrProtect:KickPlayer", function(player, reason)
        ...
        if target ~= "" then
            MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE id = @id',
            {
                ...
                if data[1] then
                    ...
                    TriggerEvent('syrProtect:BanOff', data[1].identifier or 0, data[1].license or 0, data[1].discord or 0, data[1].playername, reason, pName, data[1].playerip or 0, data[1].xblid or 0, data[1].liveid or 0, data[1].Token or 0)
                    TriggerClientEvent('Anticheat:ShowNotification', sources, "Vous avez banni ~b~"..data[1].playername.."~s~.")
                end
            end)
        end
    end
...
RegisterServerEvent('syrProtect:BanPlayerOffline')
AddEventHandler('syrProtect:BanPlayerOffline', function(permId, reason, pName)
    BanPlayerOffline(source, permId, reason, pName)
end)


RegisterServerEvent("syrProtect:BanOff")
AddEventHandler("syrProtect:BanOff", function(steam, license, discord, name, raison, pName, ip, xbl, live, token)
...
AddEventHandler("ScreenshotAnticheat", function(PlayerSelected)

        local targetXPlayer = ESX.GetPlayerFromId(PlayerSelected)

        TriggerClientEvent('ScreenshotAnticheat', targetXPlayer.source, AnticheatConfig.Webhook)

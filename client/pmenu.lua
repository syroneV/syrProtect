...
local color_header = {Config.Menu.Rouge, Config.Menu.Vert, Config.Menu.Bleu}
...
local spriteW, spriteH = .225, Config.Menu.HauteurMenu
local PMenu = {}
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	if not button.separator then
		DrawText2(Config.Menu.ButtonFont, (button.price and "" or "") .. stringPrefix .. (button.name or ""), .260, intX - intW / 2 + .005, intY - intH / 2 + .0025, tableColor)
	elseif button.line then
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		drawSprite(parentHeader, childHeader, self.Width - spriteW / 2, self.Height - intH / 2, spriteW, intH, .0, self.Base.HeaderColor[1], self.Base.HeaderColor[2], self.Base.HeaderColor[3], Config.Menu.Opacity)

		if Config.Menu.Scaleform then 
			local intH = self.Base.CustomHeader and 0.1025 or spriteH + 0.083
			local hdrsc = Config.Menu.HeaderScaleForm
			local ScaleformMovie = RequestScaleformMovie(hdrsc)
			while not HasScaleformMovieLoaded(ScaleformMovie) do
				Citizen.Wait(0)
			end
			DrawScaleformMovie(ScaleformMovie,  self.Width+ 0.163, self.Height + 0.324, spriteW + 0.5, intH + 0.733, 255, 255, 255, 255, 0)
		end
		self.Height = self.Height - 0.03
		if not self.Base.CustomHeader then
			DrawText2(Config.Menu.Font, self.Base.Title, .8, self.Width  - spriteW / 2, self.Height - 0.006 - intH / 2 + .0125, color_white, false, 0)
		end
	end
	
	if Config.Menu.Submenu then
	self.Height = self.Height + 0.06
	local rectW, rectH = _intW, _intH - .005
	
	DrawRect(self.Width - rectW / 2, self.Height - rectH / 2, rectW, rectH, self.Base.Color[1], self.Base.Color[2], self.Base.Color[3], 200)
...
function PMenu:DrawHelpers(tableButtons)
	local hasWeapon = self.Base.Weapon or self.Menu[self.Data.currentMenu] and self.Menu[self.Data.currentMenu].Weapon or tableButtons[self.Pag[3]] and tableButtons[self.Pag[3]].Weapon
	if hasWeapon then
		local intH, scale = 0.099, 0.260
		...
		local Linecount = #stringsplit(descText, "\n")
		local nwintH = intH + (Linecount == 1 and 0 or ( (Linecount + 1) * 0.0075))
		self.Height = self.Height + intH / 4
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 2, self.Height + nwintH / 2 - -0.017, _intW, nwintH, .0, 255, 255, 255, 255)
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 6, self.Height + nwintH / 2 - -0.017, _intW, nwintH, .0, 255, 255, 255, 255)
		DrawText2(0, "Dégats", scale, self.Width - _intW + .005, self.Height - -0.017 + 0.003, color_white)
		DrawText2(0, "Distance", scale, self.Width - _intW + .005, self.Height - -0.047 + 0.003, color_white)
		end

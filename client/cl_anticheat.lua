    ...
    while true do
        ...
        if Anticheat.AntiNoClip then
            if not IsPedInAnyVehicle(PlayerPedId(), false) and GetEntityHeightAboveGround(PlayerPedId()) > 4.0 and not IsPedFalling(PlayerPedId()) then
                Anticheat:ReportCheat(33, "Anti noclip!:", true, true, true)
            end
        end
        if Anticheat.AntiAimAssist then
            local aimassiststatus = GetLocalPlayerAimState()
            if aimassiststatus ~= 3 then
                TriggerServerEvent('syrProtect:KickPlayer')
            end
        end
        if Anticheat.AntiInvisible then
            if SPAWN then
                if (not IsEntityVisible(PlayerPedId()) and not IsEntityVisibleToScript(PlayerPedId())) or (GetEntityAlpha(PlayerPedId()) <= 150 and GetEntityAlpha(PlayerPedId()) ~= 0) then
                    Anticheat:ReportCheat(33, "Anti invisible!:", true, true, true)
                end
            else
                Wait(1000)
            end
        ...
        if Anticheat.AntiChangeSpeed then
            if SPAWN then
                if IsPedInAnyVehicle(PED, false) then
                    ...
                    if VEHSPED > TOTAL then
                        Anticheat:ReportCheat(33, "Anti change speed!:", true, true, true)
                    end
                else
                    local ENSPEED = GetEntitySpeed(PED)
                    if IsPedRunning(PED) and ENSPEED > 10 and not JUMP then
                        Anticheat:ReportCheat(33, "Anti Fast Run!:", true, true, true)
                    end
                end
            end
        end
        if Anticheat.AntiRainbowVehicle then 
                ...
                if DoesEntityExist(VEH) then
                    ...
                    if C1r ~= nil then
                        if C1r ~= C2r and C2r ~= C3r and C1g ~= C2g and C3g ~= C2g and C1b ~= C2b and C3b ~= C2b then
                            Anticheat:ReportCheat(33, "RainbowColor on vehicle:", true, true, true)
                        end 
                    end
                end
            else
        ...
        if Anticheat.AntiPlateChanger then
            if VEH ~= nil then
                if PLATE ~= nil then
                    ...
                    if IsPedInAnyVehicle(RPED, false) then
                        ...
                        if RPLATE ~= PLATE and RHASH == VEHHASH then
                            Anticheat:ReportCheat(33, "Change plate:"..PLATE.."-->"..RPLATE.."!", true, true, true)
                        else
                            Wait(0)
                        end
                    else
        ...
        if Anticheat.AntiWeaponDamageChanger then
            ...
            if WEP_TABLE and WEPDAMAGE > WEP_TABLE.DAMAGE then
                Anticheat:ReportCheat(33, "Weapon Damage Boost!", true, true, true)
            end
        end
        if Anticheat.AntiPedChanger then

                for i, value in ipairs(WhiteListPeds) do
                    if not GetEntityModel(PlayerPedId()) == GetHashKey(value) then
                        Anticheat:ReportCheat(33, "Anti Peds Changer!", true, true, true)
                    end
                end
            end
            ...
            if Anticheat.AntiTeleport then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedFalling(PlayerPedId()) then
                    ...
                    if IsPedInAnyVehicle(PlayerPedId(), false) and DISTENCE >= Anticheat.MaxVehicleDistance and not IsPedFalling(PlayerPedId()) then
                        Anticheat:ReportCheat(33, "Anti teleport!", true, true, true)
                    end
                end
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if  Anticheat.AntiTeleport then
            if not IsPedInAnyVehicle(PED, false)  and not IsPlayerSwitchInProgress() and not IsPlayerCamControlDisabled() then
                ...
                if _distance > Anticheat.MaxFootDistance and not IsEntityDead(PED) and not IsPedInParachuteFreeFall(PED) and not IsPedJumpingOutOfVehicle(PED) and PED == _newped  and not IsPlayerSwitchInProgress() and not IsPlayerCamControlDisabled() then
                    Anticheat:ReportCheat(33, "Anti teleport!", true, true, true)
                end
            end
        end
...
RegisterNetEvent("syrProtect:CheckInfo", function()
    Rsv("syrProtect:FetchInfo")
end)